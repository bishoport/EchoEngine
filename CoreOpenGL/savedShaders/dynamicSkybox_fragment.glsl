...
uniform vec3 sunDiskSize; // Tamao del disco solar
uniform float gradientIntensity; // Intensidad del degradado
uniform float uAuraSize; // Tamao del aura alrededor del sol
...
void main() {
    ...
    // Establecemos el intervalo de transicin cuando el sol comienza a tocar el horizonte.
    ...
    // Calculamos la diferencia en la posicin entre la posicin actual y la posicin del sol.
    float difference = length(normalizedPosition - normalizedSun);

    // No pintar el sol y usar el color uGroundColor cuando est por debajo de la lnea del ecuador.
    if(normalizedPosition.y < 0.0) {
        FragColor = vec4(uGroundColor, 1.0); // Usar el color del suelo
        return;
    }

    // Si la diferencia es menor que el tamao del disco solar ajustado por gradientIntensity, pintamos el sol blanco.
    ...
    // Aadir el aura alrededor del sol
...
//    // No pintar el sol cuando est por debajo de la lnea del ecuador.
//    if(normalizedPosition.y < 0.0) {
