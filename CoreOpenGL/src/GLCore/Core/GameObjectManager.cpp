...
namespace GLCore
{
    void GLCore::GameObjectManager::CreateEmptyGameObject()
    {
	...
	void GLCore::GameObjectManager::InstantiatePrefab(MainMenuAction action)
	{
		entt::entity entity = m_registry.create();

		// Asignar un TransformComponent a la entidad
		m_registry.emplace<TransformComponent>(entity,
			glm::vec3(0.0f), // posición inicial
			glm::quat(glm::vec3(0.0f)), // rotación inicial (como quat)
		...
		if (action == MainMenuAction::AddCube)
		{
			...
			m_registry.emplace<MeshFilterComponent>(entity,
				cube,
				PRIMIVITE_CUBE
				);
